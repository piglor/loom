...
destroy metadata or individual versions. Secret IDs are single-use and expire
after 24 hours; generate a replacement during every credential rotation.

Put the role ID and one freshly generated SecretID in the Loom deployment
secret store, then set:
...
Take a Raft snapshot at least daily and before upgrades. Copy it off the host
before removing the temporary file; a snapshot is encrypted by OpenBao's
barrier and still requires the cluster's unseal/recovery material to restore.

```sh
stamp=$(date -u +%Y%m%dT%H%M%SZ)
docker compose -f deploy/openbao/compose.yaml exec -T openbao \
...
OpenBao service first, using the matching image version and unseal material:

```sh
docker compose -p loom-openbao-restore -f deploy/openbao/compose.yaml up -d
docker compose -p loom-openbao-restore -f deploy/openbao/compose.yaml cp \
  "./raft-${stamp}.snap" openbao:/openbao/logs/restore.snap
docker compose -p loom-openbao-restore -f deploy/openbao/compose.yaml exec -T openbao \
  bao operator raft snapshot restore /openbao/logs/restore.snap
```
